...
import com.shangfudata.gatewaypay.dao.DownMchBusiInfoRespository;
import com.shangfudata.gatewaypay.dao.GatewaypayInfoRespository;
import com.shangfudata.gatewaypay.dao.UpMchBusiInfoRespository;
...
public class QueryServiceImpl implements QueryService {
    ...
    public void queryToUp() throws Exception {
        ...
        List<GatewaypayInfo> distillpayInfoList = gatewaypayInfoRespository.findByTradeState("NOTPAY");
            ...
            if ("SUCCESS".equals(gatewaypayInfo.getStatus())) {
                ...
                if (!(responseInfo.getTrade_state().equals(gatewaypayInfo.getTrade_state()))) {
                    ...
                    if ("SUCCESS".equals(status)) {
                        //将订单信息表存储数据库
                        gatewaypayInfoRespository.updateSuccessTradeState(trade_state, err_code, err_msg, out_trade_no);

                        //如果交易状态为成功，做清分
                        if("SUCCESS".equals(trade_state)){
                            Distribution(gatewaypayInfoRespository.findByOutTradeNo(out_trade_no));
                        }
                    } else if ("FAIL".equals(status)) {
                        gatewaypayInfoRespository.updateFailTradeState(status, code, message, out_trade_no);
                    }

                    noticeService.ToDown(gatewaypayInfoRespository.findByOutTradeNo(out_trade_no));
                }
            }
        }
    }
    ...
    public String downQuery(String gatewaypayInfoToJson) {
        ...
        // 异常处理
        dataValidationUtils.queryGatepayException(gatewaypayInfo , responseMap);
        // 异常处理后判断是否需要返回
    ...
    public void Distribution(GatewaypayInfo gatewaypayInfo){
        ...
        String down_mch_id = gatewaypayInfo.getDown_mch_id();
        String mch_id = gatewaypayInfo.getMch_id();

        //拿到上下游商户id后查询两张商户业务表
        DownMchBusiInfo downMchBusiInfo = downMchBusiInfoRespository.findByDownMchIdAndBusiType(down_mch_id, "gatewaypay");
        //System.out.println("下游业务信息：："+downMchBusiInfo);
        UpMchBusiInfo upMchBusiInfo = upMchBusiInfoRespository.findByMchIdAndBusiType(mch_id, "gatewaypay");
