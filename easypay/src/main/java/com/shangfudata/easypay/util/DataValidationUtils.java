...
import com.shangfudata.easypay.exception.MyException;
...
public class DataValidationUtils {
    ...
    public String isMust(Map<String, String> map) {
        if (map.containsKey("down_sp_id")&&map.containsKey("down_mch_id")&&map.containsKey("out_trade_no")&&map.containsKey("body")
                &&map.containsKey("total_fee")&&map.containsKey("card_type")&&map.containsKey("card_name")&&map.containsKey("card_no")
                &&map.containsKey("id_type")&&map.containsKey("id_no")&&map.containsKey("bank_code")&&map.containsKey("bank_name")
                &&map.containsKey("bank_mobile")&&map.containsKey("notify_url")&&map.containsKey("nonce_str"))
        {
            return "1";
        }
           else
    ...
    public DataValidationUtils urlConnection(String url) throws MalformedURLException, IOException {
        ...
        urlProxy.openConnection().connect();
    ...
    public void processMyException(EasypayInfo easypayInfo, Map responseMap) {
        ...
        try {
            dataValidationUtils.bankCardValid(easypayInfo.getCard_no()).cardValid(easypayInfo.getId_type(),
                    ...
                    mobileNumberValid(easypayInfo.getBank_mobile()).nonceStrValid(easypayInfo.getNonce_str()).urlConnection(easypayInfo.getNotify_url());
            ...
            responseMap.put("status", "FAIL");
            responseMap.put("message", "贷记卡参数不能为空");
            ...
            responseMap.put("status", "FAIL");
            responseMap.put("message", "url 格式错误");
        } catch (IOException e) {
            responseMap.put("status", "FAIL");
            responseMap.put("message", "url 无法正常访问");
        }
    }
     ...
     * EasyPay 下游请求参数异常处理方法
     */
    public void queryEasypayException(EasypayInfo easypayInfo, Map responseMap) {
        // 数据效验
        // 异常处理
        try {
